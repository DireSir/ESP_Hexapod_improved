...
class HexapodControllerGUI(QMainWindow):
  ...
  def _init_ui(self):
    ...
    self.terminal_log_area.setStyleSheet("background-color: #f0f0f0; color: #333; font-family: Consolas, monospace;")
    ...
    if msg_type == "full_state_response" and payload:
      ...
      if "battery" in payload:
        ...
        self.actual_battery_voltage_label.setText(
          f"{v:.2f}V ({p}%)" if isinstance(v,float) and p else f"{v:.2f}V" if isinstance(v,float) else f"{v}V")

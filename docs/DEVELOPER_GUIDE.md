...
The Hexapod's firmware is developed using the Arduino IDE with the ESP32 board support package. If you prefer using a CLI instead, refer to the [Linux User's guide](./).
...
### 1.5. Configure the Default Wi-Fi Credentials
1. Open the `config.h` file from the project.
2. Modify the `WIFI_NETWORKS` list to match your Wi-Fi network:
  ```cpp
  static const WifiCred WIFI_NETWORKS[] = {
    { "my-wifi-network", "my-wifi-password" },
    { "my-wifi-network-two", "my-wifi-password" },
  };
  ```
...
* **`config.h`**: Stores the default Wi-Fi networks as well as the default feedback LED port.
...
* **`config.h`**: **Wi-Fi Credentials (SSID & Password).** Should be changed if you want to add/remove a default network.
...
### Your (truly) Dire's additions, impressions & rules
---
**Adressing the elephant in the room - indendation:**

If you have already taken a look at this codebase, you may have noticed that I prefer to use 2 spaces for indenation, whereas the original code swtiched between using 2 and 4 spaces. I could not tolerate this, and thus, I re-indented everything to 2 spaces. I would kindly ask You to use two spaces in this project too.

I know that the question of indentation is very much a heated one even today, but in everything I do, I strive to follow a philosophy of measured minimalism; *We do what we must, but we fear doing too much and having to correct the mistakes in what we could have avoided entirely later.*

In this case, my logic is as follows: If the purpose of indenting our code is purely to make it readeable, then two spaces achieve that just fine, while also being as unobtrusive as possible. Torvalds' school of thought objects to this notion, saying that four looks better and is more established than two, and, crucially, *that preserving space isn't a concern because if you have more than three levels of indentation, you're 'cooked anyway'*. I *fundamentally* disagree with this, and there are many arguments I could employ, but for Your, reader's time, I will only state my most fundamental counter-argument: *A system must not rely on assumptions to work. A system that relies on a generalized assumption that is simply untrue in far too many cases to work, hell, even a notion of such a system, especially when this system lies at the very foundation of programming, is outragous.*
...
###### *YourDire's very personal note: Blah-blah-blah above. Just don't be stupid.*
...
### Wi-Fi Connection Issues (During Development)
  * **Check `config.h`:** Ensure the default networks are available; If they are not, make sure to connect to a network using the CLI.
